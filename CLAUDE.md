...
- **`tools/`** — not loaded by the game: the HorizonXI wiki harvester and the
  parsed item data the gear ladders are scored from (see `tools/README.md`)
...
**Idle modes (BRD):** `BRD.lua` holds an `IdleModes` registry — an ordered array
of `{ Cmd, Set, WHM, Label }`. Exactly one mode is active; `HandleDefault`
equips its full set, then its optional two-slot subjob set when the subjob is
white mage. `/brd mit` and `/brd enmity` switch; `/brd help` (or a bare `/brd`)
lists every command the profile accepts and marks the active mode, and
`/brd modes` lists just the modes. The help listing is generated by walking the
